...
@RUNNERS.register("KFoldRunnerSpeedUp")
class KFoldRunnerSpeedUp:
    ...
    def run(self, cfg, resume_path=None):
        ...
        dataloader_kwargs = {
            ...
            'num_workers': min(os.cpu_count() // 2, 2),  # 从4改为2，Windows更稳定
            'pin_memory': True,  # 启用内存锁页
            'persistent_workers': True,  # 保持worker进程存活（核心提速）
            'prefetch_factor': 2,  # Windows下prefetch_factor设为2，避免内存溢出
        }
